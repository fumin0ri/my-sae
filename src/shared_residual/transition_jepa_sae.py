...
def main() -> None:
    ...
    if x.ndim != 3 or x.shape[1] != 10:
        raise ValueError("transition JEPA requires [windows, 10, d_model]")
    ...
    if standard_checkpoint.get("split") != expected_split:
        raise ValueError(
            "standard SAE checkpoint must use the same locked group split"
        )
    loader_options = {
